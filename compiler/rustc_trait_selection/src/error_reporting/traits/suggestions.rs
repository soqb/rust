...
pub fn suggest_restriction<'tcx, G: EmissionGuarantee>(
    ...
    if hir_generics.where_clause_span.from_expansion()
        ...
        || projection.is_some_and(|projection| {
            (tcx.is_impl_trait_in_trait(projection.ctor.expect_def())
                && !tcx.features().return_type_notation())
                || tcx
                    .lookup_stability(projection.ctor.expect_def())
...
impl<'a, 'tcx> TypeErrCtxt<'a, 'tcx> {
                    ...
                    match ty.kind() {
                        ...
                        ty::Alias(ty::Opaque, ty::AliasTy { ctor, .. }) => {
                                ...
                                })
                            {
                                ...
                                let msg = msg();
                                err.span_note(ctor.span(tcx), msg);
                            }
                        }
        ...
        for diff in type_diffs {
            ...
            let projection = ty::Binder::dummy(ty::PredicateKind::Clause(
                ty::ClauseKind::Projection(ty::ProjectionPredicate {
                    projection_term: ty::AliasTerm::new_from_args(self.tcx, proj.ctor, args),
                    term: ty.into(),
                }),
            ));
    ...
    pub(super) fn suggest_desugaring_async_fn_in_trait(
        ...
        let Some(ty::ImplTraitInTraitData::Trait { fn_def_id, opaque_def_id }) =
            self.tcx.opt_rpitit_info(alias_ty.ctor.expect_def())
        else {
            return;
        };
...
fn point_at_assoc_type_restriction<G: EmissionGuarantee>(
    ...
    let name = tcx.item_name(proj.def_id());
